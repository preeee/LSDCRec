...
def parse_args():
    ...
    parser.add_argument("--no_cuda", action="store_true")       # 如果命令行中出现 --no_cuda，则 args.cuda_condition 为 False，表示不使用GPU；否则使用GPU
    ...
    parser.add_argument("--num_workers", default=4, type=int)   # 用于数据加载的线程数，是DataLoader的参数

    parser.add_argument("--seed", default=42, type=int)
    parser.add_argument("--weight_decay", default=0.0, type=float, help="weight_decay of adam")
    parser.add_argument("--adam_beta1", default=0.9, type=float, help="adam first beta value")  # Adam优化器
    ...
    # model args
    parser.add_argument("--model_type", default='FMRec', type=str)
    parser.add_argument("--max_seq_length", default=50, type=int)       # 序列的最大长度,论文中是50
    ...
    parser.add_argument('--d_k', type=int, default=16, help='dimension of each head')
    parser.add_argument('--init_val', type=float, default=1, help='initial value for hyper-param opt')
    parser.add_argument('--is_relative', type=bool, default=True, help='......')
    parser.add_argument("--feed_forward_hidden_size", default=64, type=int, help="dimension of feed forward layer")
    parser.add_argument('--residual', type=bool, default=True, help='if residual')

    ######## SLIME4Rec
    ######## 2025-2-14
    parser.add_argument('--filter_mixer', type=str, default='G', help='Types of filter mixer')
    parser.add_argument("--dynamic_ratio", default=0.8, type=float, help="dynamic ratio")
    parser.add_argument('--slide_mode', type=str, default='four', help='Types of slide mode')


    ########## 2025.3.13 用户意图
    # parser.add_argument('--intent_labels_path', type=str, default='./data/intent_labels.npy',
    #                     help='Path to save/load intent labels')
    ...
    # 2024-9-4  增加mswtrec
    if args.model_type.lower() == 'mswtrec':
        parser.add_argument("--alpha", default=0.9, type=float)



    # 2024-5-9 17:05 增加wtrec
    if args.model_type.lower() == 'wtrec':
        parser.add_argument("--alpha", default=0.9, type=float)
